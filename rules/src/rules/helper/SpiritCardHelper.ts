...
export class SpiritCardHelper extends MaterialRulesPart {
    ...
    if (isMoveItem(move)) {
      ...
      if (previousLocation) {
        ...
        if (previousLocation.type === LocationType.BearEliteCard) {
          this.memorize(
            Memory.SpiritCardsToTake,
            spiritCardsToTake.filter((card) => card !== 'bear')
          )
          ...
          this.memorize(
            Memory.SpiritCardsToTake,
            spiritCardsToTake.filter((card) => card !== 'fox')
          )
          ...
          this.memorize(
            Memory.SpiritCardsToTake,
            spiritCardsToTake.filter((card) => card !== 'owl')
          )
  ...
  get bearSpiritCardInDeck() {
    return this.material(MaterialType.SpiritCard)
      .location(LocationType.BearEliteCardsDeck)
  ...
  get foxSpiritCardInDeck() {
    return this.material(MaterialType.SpiritCard)
      .location(LocationType.FoxEliteCardsDeck)
      .maxBy((item) => item.location.x ?? 0)
  }
  ...
  get owlSpiritCardInDeck() {
    return this.material(MaterialType.SpiritCard)
      .location(LocationType.OwlEliteCardsDeck)
      .maxBy((item) => item.location.x ?? 0)
  }
  ...
  getSpiritCardNearToDruidAfterItemMove(id: SpiritCardId, locationId: number): MaterialMove[] {
    ...
    const druidId: SpiritCardId = this.material(MaterialType.SpiritCard)
