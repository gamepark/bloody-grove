...
export class EndOfGameHelper extends MaterialRulesPart {
  ...
  onePlayerHaveAllGroves(): boolean {
    const groveMajorities = this.material(MaterialType.Cube)
  ...
  getScore(player: PlayerColor): number {
    ...
    const druidTransformation = this.material(MaterialType.SpiritCard)
    ...
    playerSpiritCardsUnderGroves.forEach((card) => {
      const otherPlayerCardsInGrove = this.material(MaterialType.SpiritCard)
        .location((loc) => loc.type === LocationType.PlayerSpiritUnderGroveLayout && loc.id === card.location.id)
        .player(player)
        .getItems()
