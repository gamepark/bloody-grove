...
export class BloodyGroveSetup extends MaterialGameSetup<PlayerColor, MaterialType, LocationType, BloodyGroveOptions> {
  ...
  setupPlayers() {
    this.players.forEach((player) => {
      ...
      this.material(MaterialType.SpiritCard).createItems(
        shuffle(baseSpiritCards).map((id) => ({ id: { front: id, back: player * 100 + SpiritType.Base }, location: { type: LocationType.PlayerDeck, player } }))
      )
  ...
  setupElderSpiritCards() {
    this.material(MaterialType.SpiritCard).createItems(
      shuffle(elderSpiritRedCards).map((id) => ({ id: { front: id, back: SpiritType.ElderRed }, location: { type: LocationType.ElderSpiritCardsDeck } }))
    )
    this.material(MaterialType.SpiritCard).createItems(
      shuffle(elderSpiritCards).map((id) => ({ id: { front: id, back: SpiritType.Elder }, location: { type: LocationType.ElderSpiritCardsDeck } }))
    )
    this.material(MaterialType.SpiritCard)
      .location(LocationType.ElderSpiritCardsDeck)
      .maxBy((item) => item.location.x ?? 0)
      .moveItems({ type: LocationType.ElderSpiritCardsRiver })
    this.material(MaterialType.SpiritCard)
      .location(LocationType.ElderSpiritCardsDeck)
      .maxBy((item) => item.location.x ?? 0)
      .moveItems({ type: LocationType.ElderSpiritCardsRiver })
